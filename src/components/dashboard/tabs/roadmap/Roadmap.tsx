...
export function Roadmap() {
  ...
  const calculateColumnTotalTime = (milestones: Milestone[]) => {
    if (!milestones || milestones.length === 0) return { value: 0, unit: 'days' as const };
  ...
  const formatTimeEstimateAsRange = (estimate: { value: number, unit: 'days' | 'weeks' | 'months' }) => {
    if (estimate.value <= 1) {
      ...
      <div className="flex-1 overflow-hidden flex p-4 space-x-4 bg-gray-50">
        {columns.map((column) => {
          ...
          return (
            ...
            >
              {/* Column header */}
              <div className={`p-3 bg-gradient-to-r from-indigo-500 to-blue-500 text-white rounded-t-lg`}>
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-medium">{column.name}</h3>
                    <div className="flex items-center text-xs text-white/90 mt-0.5">
                      <Clock size={12} className="mr-1" />
                      <span>{formatTimeEstimateAsRange(columnTotalTime)}</span>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-3">
                {milestones?.length ? (
                  milestones.map((milestone) => {
                    const totalEstimate = calculateTotalTime(milestone.tasks);
                    ...
                    return (
                      ...
                      >
                        {/* Milestone header */}
                        <div className="p-3" onClick={() => toggleMilestone(milestone.id)}>
                          ...
                          <div className="flex items-center mb-2 text-sm text-gray-600">
                            ...
                            <span>
                              {formatTimeEstimateAsRange(totalEstimate)}
                            </span>
                          </div>

                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2 py-0.5 rounded-full border ${getPriorityBadge(milestone.priority)}`}>
