...
export default function ProjectForm() {
	...
	async function sendPrompt() {
		...
		try {
			...
			let validFeatureData = JSON.parse(featureData.body);

			localStorage.setItem("Features", JSON.stringify(validFeatureData.message));
			// console.log("Feature Response:", validFeatureData.message);
			...
			const stackResponse = await fetch(
				"https://dr8rn41bv4.execute-api.us-east-2.amazonaws.com/GeminiStage",
				{
					...
					body: JSON.stringify({
						formData: formData,
						featureContext: validFeatureData.message,
			...
			let validStackData = JSON.parse(stackData.body);

			localStorage.setItem("Stack", JSON.stringify(validStackData.message));
			// console.log("Stack Response:", validStackData.message);
			...
			const roadmapResponse = await fetch(
				"https://dr8rn41bv4.execute-api.us-east-2.amazonaws.com/GeminiStage",
				{
					...
					body: JSON.stringify({
						formData: formData,
						featureContext: validFeatureData.message,
						stackContext: validStackData.message,
			...
			let validRoadmapData = JSON.parse(roadmapData.body);

			localStorage.setItem("Roadmap", JSON.stringify(validRoadmapData.message));
			// console.log("Roadmap Response:", validRoadmapData.message);
